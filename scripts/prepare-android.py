...
# Patch WryActivity.kt to make the id property open so MainActivity can
# override it. tao-0.35.3 calls getId() via JNI on the activity instance
...
# allows MainActivity to override it, generating getId() directly on
# MainActivity.
#
# WryActivity.kt is generated by wry's build.rs during `cargo tauri android
# build`, not during `cargo tauri android init`. So the generated file may
# not exist yet when prepare-android.py runs. We patch both:
# 1. The generated file (if it exists from a previous local build)
# 2. The wry crate's Kotlin template in the cargo registry, so the next
#    `cargo tauri android build` generates the patched version.
wry_activity = android / "app" / "src" / "main" / "java" / "music" / "virya" / "signal" / "generated" / "WryActivity.kt"
if wry_activity.is_file():
...
# Patch the wry crate's Kotlin template so wry's build.rs generates
# `open var id` instead of `var id`. The template lives in the cargo
# registry at a path derived from the wry version in Cargo.lock.
...
if cargo_lock.is_file():
    ...
    for i, line in enumerate(lock_lines):
        if line.strip().startswith("name = \"wry\""):
            wry_version = lock_lines[i + 1].strip().split('"')[1]
            break
    if wry_version is not None:
        cargo_registry = Path.home() / ".cargo" / "registry" / "src"
        wry_template = None
        if cargo_registry.is_dir():
            for registry_dir in cargo_registry.iterdir():
                candidate = registry_dir / f"wry-{wry_version}" / "src" / "android" / "kotlin" / "WryActivity.kt"
                if candidate.is_file():
                    wry_template = candidate
                    break
        if wry_template is not None:
            template_text = wry_template.read_text(encoding="utf-8")
            if "open var id" not in template_text:
                template_text = template_text.replace("var id: Int = 0", "open var id: Int = 0", 1)
                wry_template.write_text(template_text, encoding="utf-8")
        else:
            print(f"WARNING: wry-{wry_version} template not found in cargo registry; "
                  f"WryActivity.kt will use the unpatched template")
else:
    print("WARNING: Cargo.lock not found; skipping wry template patch")

# Patch MainActivity.kt to override the id property. This generates getId()
