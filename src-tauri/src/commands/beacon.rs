...
/// Drops everything this process holds for a Beacon identity that is going away.
/// `beacon_lock` keeps a queued App Link on purpose — locking is temporary and
/// the invitation is still the member's to spend. Leaving and logging out are
/// not: a capability that survived either would be offered to whoever sets this
/// device up next, as an invitation into a relationship that no longer exists.
async fn clear_beacon_session_state(state: &State<'_, AppState>) {
    *state.beacon_session.write().await = None;
    *state.beacon_pin.write().await = None;
    *state.beacon_vault_password.write().await = None;
    *state.pending_beacon_confirmation.lock().await = None;
