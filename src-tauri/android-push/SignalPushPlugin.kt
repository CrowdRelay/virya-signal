...
/// The fan vault password is sealed under this key. It is generated inside the
/// keystore and marked non-exportable, so the sealed file is worthless on any
/// other device and worthless on this one once the key is deleted.
private const val DEVICE_SECRET_ALIAS = "virya-fan-vault-v1"
private const val DEVICE_SECRET_TRANSFORMATION = "AES/GCM/NoPadding"
private const val DEVICE_SECRET_TAG_BITS = 128
private const val DEVICE_SECRET_IV_BYTES = 12
private const val MAX_DEVICE_SECRET_BYTES = 4096
...
class SignalPushPlugin(private val activity: Activity) : Plugin(activity) {
    ...
    @Command
    fun sealDeviceSecret(invoke: Invoke) {
        val args = invoke.parseArgs(DeviceSecretArgs::class.java)
        val plaintext = args.value
        if (plaintext.isEmpty() || plaintext.length > MAX_DEVICE_SECRET_BYTES) {
            invoke.reject("invalid_device_secret")
            return
        }
        val key = runCatching { loadOrCreateDeviceKey() }.getOrNull()
        if (key == null) {
            invoke.reject("device_secret_unavailable")
            return
        }
        val sealed = runCatching {
            val cipher = Cipher.getInstance(DEVICE_SECRET_TRANSFORMATION)
            cipher.init(Cipher.ENCRYPT_MODE, key)
            // The IV is generated by the keystore, never reused, and travels
            // in front of the ciphertext: GCM is broken outright by a repeated
            // one, so it must not be chosen here.
            val iv = cipher.iv
            val body = cipher.doFinal(plaintext.toByteArray(Charsets.UTF_8))
            Base64.encodeToString(iv + body, Base64.NO_WRAP)
        }.getOrNull()
        if (sealed == null) {
            invoke.reject("device_secret_seal_failed")
            return
        }
        val result = JSObject()
        result.put("value", sealed)
        invoke.resolve(result)
    }

    @Command
    fun openDeviceSecret(invoke: Invoke) {
        val args = invoke.parseArgs(DeviceSecretArgs::class.java)
        val sealed = runCatching { Base64.decode(args.value, Base64.NO_WRAP) }.getOrNull()
        if (sealed == null || sealed.size <= DEVICE_SECRET_IV_BYTES ||
            sealed.size > MAX_DEVICE_SECRET_BYTES
        ) {
            invoke.reject("invalid_device_secret")
            return
        }
        val key = runCatching { existingDeviceKey() }.getOrNull()
        if (key == null) {
            // The key is gone — a restore onto another device, a "clear
            // credentials", an uninstall. The sealed bytes can never be opened
            // again, and saying so is what sends the app back to the PIN
            // instead of retrying forever.
            invoke.reject("device_secret_key_missing")
            return
        }
        val plaintext = runCatching {
            ...
            cipher.init(
                ...
                key,
                GCMParameterSpec(
                    DEVICE_SECRET_TAG_BITS,
                    sealed.copyOfRange(0, DEVICE_SECRET_IV_BYTES),
                ),
            )
            String(
                cipher.doFinal(sealed.copyOfRange(DEVICE_SECRET_IV_BYTES, sealed.size)),
                Charsets.UTF_8,
            )
        }.getOrNull()
        if (plaintext.isNullOrEmpty()) {
            invoke.reject("device_secret_open_failed")
            return
        }
        val result = JSObject()
    ...
    @Command
    fun clearDeviceSecret(invoke: Invoke) {
        runCatching {
            val store = KeyStore.getInstance("AndroidKeyStore").apply { load(null) }
            if (store.containsAlias(DEVICE_SECRET_ALIAS)) {
                store.deleteEntry(DEVICE_SECRET_ALIAS)
            }
        }
        invoke.resolve()
    }

    private fun existingDeviceKey(): SecretKey? {
        val store = KeyStore.getInstance("AndroidKeyStore").apply { load(null) }
        return store.getKey(DEVICE_SECRET_ALIAS, null) as? SecretKey
    }

    private fun loadOrCreateDeviceKey(): SecretKey? {
        existingDeviceKey()?.let { return it }
        ...
        val spec = KeyGenParameterSpec.Builder(
            ...
            .setKeySize(256)
            // No user-authentication requirement on purpose: the point of this
            // key is that a mailed link opens the app with nothing else asked
            // of the fan. The device lock screen and the app sandbox are the
            // perimeter, and the PIN stays available for anyone who wants a
            // secret in their head instead.
