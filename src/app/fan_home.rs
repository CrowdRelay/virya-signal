...
/// Synesthesia progress card — shown only to fans who already started or
/// completed the interactive album. Rendered below the "carry the signal"
/// referral block on the Signal tab so the primary CTA (next show + referral
/// benefits) comes first, and the opt-in side experience follows.
/// Synesthesia is a side album experiment, not a primary Home CTA.
#[component]
fn SynesthesiaHomeCard(
    home: RwSignal<Option<FanHomeData>>,
    error: RwSignal<Option<String>>,
) -> impl IntoView {
            ...
            view! {
                <Show when=move || show_synesthesia>
                    <article class="home-action-card synesthesia-home-card">
                            ...
                            <strong>{if synesthesia.completed { tr("journey_completed") } else { tr("continue_the_journey") }}</strong>
